  ...
  return (
    <div className="flex flex-col h-full bg-transparent w-full">
      ...
      <div className="px-3.5 pt-4 pb-3 w-full">
        ...
        <div className="mt-3.5 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="bg-white dark:bg-[#0E281C]/80 backdrop-blur-xl border border-slate-200/80 dark:border-emerald-500/20 rounded-2xl p-4 shadow-sm dark:shadow-lg">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-white/60 mb-1">
                {s.icon}
                <p className="text-slate-500 dark:text-white/60 text-xs font-medium">{s.label}</p>
              </div>
              <p className="font-display font-black text-[#0F172A] dark:text-white text-2xl mt-0.5">{s.value}</p>
              <p className={"text-xs mt-0.5 font-bold " + (s.up ? "text-emerald-700 dark:text-[#34D399]" : "text-red-500 dark:text-red-400")}>
                {s.change}
              </p>
            </div>
          ))}
      ...
      <div className="flex-1 overflow-y-auto px-3.5 pb-20 pt-2 space-y-4 w-full">
        ...
        {!loading && hasNoData && (
          <div className="bg-white dark:bg-[#0E281C]/85 backdrop-blur-xl rounded-3xl p-8 text-center border border-slate-200/80 dark:border-emerald-500/20 shadow-md dark:shadow-2xl mb-4">
            <div className="w-14 h-14 rounded-2xl bg-emerald-100 dark:bg-[#10B981]/20 border border-emerald-300 dark:border-[#10B981]/30 flex items-center justify-center mx-auto mb-3 text-emerald-700 dark:text-[#34D399]">
              <BarChartIcon size={28} />
            </div>
            <p className="font-display font-bold text-[#0F172A] dark:text-white text-lg">
        ...
        {/* Responsive 2-column Grid for Charts on Desktop */}
          ...
          <div className="bg-white dark:bg-[#0E281C]/85 backdrop-blur-xl rounded-3xl p-5 shadow-md dark:shadow-2xl border border-slate-200/80 dark:border-emerald-500/20 text-[#0F172A] dark:text-white">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <BarChartIcon size={18} className="text-emerald-600 dark:text-[#34D399]" />
                <h2 className="font-display font-bold text-[#0F172A] dark:text-white text-base">
                  {isBn ? "সাপ্তাহিক সিল ট্রেন্ড" : "Weekly Stamp Trend"}
                </h2>
              </div>
              <span className="text-slate-400 dark:text-white/40 text-xs">{isBn ? "গত ৭ দিন" : "Past 7 days"}</span>
            </div>
            <div className="flex items-end gap-1.5 h-32">
              {weeklyData.map((d, i) => (
                <div key={d.day || i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <p className="text-slate-400 dark:text-white/50 text-[10px]">{d.day}</p>
                  <p className="font-display font-black text-emerald-700 dark:text-[#34D399] text-xs">{d.stamps}</p>
                </div>
              ))}
            </div>
            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-white/10 flex items-center justify-between text-xs text-slate-600 dark:text-white/70">
              <span>
                {isBn ? "মোট: " : "Total: "}
                <strong className="text-emerald-700 dark:text-[#34D399]">{stats.stampsThisWeek}</strong>{" "}
                {isBn ? "সিল এই সপ্তাহে" : "stamps this week"}
              </span>
              <span>
                {isBn ? "গড়: " : "Avg: "}
                <strong className="text-emerald-700 dark:text-[#34D399]">
                  {weeklyData.length > 0
        ...
        {retentionData.length > 0 && (
          <div className="bg-white dark:bg-[#0E281C]/85 backdrop-blur-xl rounded-3xl p-5 shadow-md dark:shadow-2xl border border-slate-200/80 dark:border-emerald-500/20 text-[#0F172A] dark:text-white">
            <div className="flex items-center gap-2 mb-1">
              <TrendingUpIcon size={18} className="text-emerald-600 dark:text-[#34D399]" />
              <h2 className="font-display font-bold text-[#0F172A] dark:text-white text-base">
                {isBn ? "কাস্টমার রিটেনশন ফানেল" : "Customer Retention Funnel"}
              </h2>
            </div>
            <p className="text-slate-500 dark:text-white/60 text-xs mb-4">
              {isBn
              ...
              {retentionData.map((d, i) => (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-[#0F172A] dark:text-white text-sm font-medium">{d.label}</p>
                    <div className="flex items-center gap-2">
                      <span className="font-display font-bold text-emerald-700 dark:text-[#34D399]">
                        {d.value} {isBn ? "জন" : "users"}
                      </span>
                      <span className="text-slate-400 dark:text-white/40 text-xs">({d.pct}%)</span>
                    </div>
                  </div>
                  <div className="h-2 bg-slate-100 dark:bg-[#071D13] rounded-full overflow-hidden border border-slate-200 dark:border-white/10">
                    <div
            ...
            <div className="mt-4 p-3 bg-slate-50 dark:bg-[#071D13] rounded-2xl border border-slate-200 dark:border-emerald-500/20 flex items-center gap-2">
              <CheckIcon size={14} className="text-emerald-600 dark:text-[#34D399] flex-shrink-0" />
          ...
          <div className="bg-white dark:bg-[#0E281C]/85 backdrop-blur-xl rounded-3xl p-5 shadow-md dark:shadow-2xl border border-slate-200/80 dark:border-emerald-500/20 text-[#0F172A] dark:text-white">
            ...
            <div className="space-y-3">
              {topCustomers.map((c, i) => {
                return (
                  <div key={c.id || i} className="flex items-center gap-3 p-2 rounded-2xl bg-slate-50 dark:bg-[#071D13] border border-slate-200/80 dark:border-white/10">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs flex-shrink-0 bg-slate-200 dark:bg-white/10">
                      {i === 0 ? (
                        <TrophyIcon size={16} className="text-[#F59E0B]" />
                      ) : i === 1 ? (
                        <MedalIcon size={16} className="text-emerald-600 dark:text-[#34D399]" />
                      ) : i === 2 ? (
                        <MedalIcon size={16} className="text-amber-500" />
                      ) : (
                        <span className="text-slate-600 dark:text-white/60 font-mono">{i + 1}</span>
                      )}
                    </div>
                    <div className="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-[#10B981]/20 border border-emerald-300 dark:border-[#10B981]/30 flex items-center justify-center font-bold text-xs text-emerald-800 dark:text-[#34D399]">
                      {c.name ? c.name.slice(0, 1) : (isBn ? "ক" : "C")}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[#0F172A] dark:text-white truncate">{c.name}</p>
                      <p className="text-xs text-slate-500 dark:text-white/50">
                        {c.totalVisits || c.stamps} {isBn ? "বার মোট ভিজিট" : "total visits"}
                      </p>
                    </div>
                    <span className="text-xs bg-emerald-50 dark:bg-[#34D399]/20 text-emerald-800 dark:text-[#34D399] border border-emerald-200 dark:border-[#34D399]/30 px-2.5 py-1 rounded-full font-bold">
